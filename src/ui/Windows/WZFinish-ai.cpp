...
BOOL CWZFinish::OnInitDialog()
{
  ...
  // For export operations, keep Back button visible but disabled until completion
  // For other operations (Compare, Merge, Synchronize), hide Back button
  if (bIsExport) {
...
LRESULT CWZFinish::OnExecuteThreadEnded(WPARAM , LPARAM )
{
  ...
  // Enable Finish button, and Back button for export operations
  if (bIsExport) {
