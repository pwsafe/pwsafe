    ...
    switch (wNotify) {
      case CBN_SELENDOK:
        if (nID == m_FLD_ComboID) {
          ...
          if (m_pComboBox->GetItemData(index) != FT_END) {
            iLastSelectedOK = index;
            SetField(m_iItem);
          } else {
            m_pComboBox->SetCurSel(iLastSelectedOK);
          }
        } else {
...
void CPWFilterLC::RemoveFilter()
{
  ...
  if (m_iType == DFTYPE_MAIN) {
    ...
    else if (m_bATTACHMENT_Set && st_fldata.ftype == FT_ATTACHMENT) {
      // We are deleting the Attachment so put option back in other Comboboxes
...
bool CPWFilterLC::SetField(const int iItem)
{
  ...
  if (m_iType == DFTYPE_MAIN) {
    ...
    if (vlast_ft[iItem] == FT_ATTACHMENT && ft != FT_ATTACHMENT)
      m_bATTACHMENT_Set = false;
...
bool CPWFilterLC::GetCriterion()
{
  ...
  switch (st_fldata.mtype) {
    ...
    case PWSMatch::MT_MEDIATYPE:
      m_fmediatype.m_add_present = vAddPresent[m_iItem];
...
void CPWFilterLC::DrawComboBox(const int iSubItem, const int index)
{
  ...
  switch (iSubItem) {
    case FLC_FLD_COMBOBOX:
    {
      ...
      int max_chars(0);
      for (size_t i = 0; i < vWCFcbx_data.size(); i++) {
        max_chars = max(max_chars, vWCFcbx_data[i].cs_text.GetLength());

        // Save index to separators
        if (vWCFcbx_data[i].ftype == FT_END)
      ...
      for (size_t i = 0; i < vSeparators.size(); i++) {
        vWCFcbx_data[vSeparators[i]].cs_text = cs_separator;
      }
    ...
    case FLC_LGC_COMBOBOX:
      for (size_t i = 0; i < vLcbx_data.size(); i++) {
        m_pComboBox->AddString(vLcbx_data[i].cs_text);
        m_pComboBox->SetItemData(i, vLcbx_data[i].ltype);
      }
      break;
    default:
      ASSERT(0);
  ...
  //for (int i = 0; i < m_pComboBox->GetCount(); i++) {
  //  if (m_pComboBox->GetItemData(i) == FT_END) {
  //    m_pComboBox->SetItemHeight(i, 5);
  //  }
  //}
