...
CRichEditCtrlExtn::CRichEditCtrlExtn()
  : m_pfcnNotifyLinkClicked(NULL), m_bNoClipboard(false)
{
}
...
void CRichEditCtrlExtn::PasteSpecial(UINT nClipFormat, DWORD dvAspect, HMETAFILE hMF)
{
  if (m_bNoClipboard)
    return;

  CRichEditCtrl::PasteSpecial(nClipFormat, dvAspect, hMF);
}

BOOL CRichEditCtrlExtn::Undo()
{
  if (m_bNoClipboard)
    return TRUE;

  return CRichEditCtrl::Undo();

}

BOOL CRichEditCtrlExtn::Redo()
{
  if (m_bNoClipboard)
    return TRUE;

  return CRichEditCtrl::Redo();
