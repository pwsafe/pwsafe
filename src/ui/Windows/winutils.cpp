...
void WinUtil::MoveControlDelta(CWnd& ctl, int dx, int dy) {
  CWnd* parent = ctl.GetParent();
  CRect rc;
  ctl.GetWindowRect(&rc);
  int newX = rc.left + dx;
  int newY = rc.top + dy;
  if (dx && dy)
    rc.MoveToXY(newX, newY);
  else if (dx)
    rc.MoveToX(newX);
  else if (dy)
    rc.MoveToY(newY);
...
bool WinUtil::RemoveControlRows(CWnd& parent, int nIdFirstCtl, int cColumns, int cRows, const std::vector<int>& vRowsToRemove, int& cyVertDistanceTotal)
{
  cyVertDistanceTotal = 0;
  if (vRowsToRemove.empty())
    return true;
  ASSERT(&parent);
  if (!&parent)
    return false;
  ...
  for (int row = 0; row < cRows; row++) {
    auto itFind = std::find(vRowsToRemove.begin(), vRowsToRemove.end(), row);
    if (itFind != vRowsToRemove.end()) {
      const int idRowFirstCtl1 = nIdFirstCtl + (row * cColumns);
      auto wnd1 = parent.GetDlgItem(idRowFirstCtl1);
      if (!wnd1) {
        ASSERT(FALSE);
        bSuccess = false;
        continue;
      ...
      if (row + 1 < cRows) {
        const int idRowFirstCtl2 = nIdFirstCtl + ((row + 1) * cColumns);
        auto wnd2 = parent.GetDlgItem(idRowFirstCtl2);
        if (!wnd2) {
          ASSERT(FALSE);
          bSuccess = false;
          continue;
        ...
        cyVertDistanceTotal += rc2.top - rc1.top;
      }
      else {
        cyVertDistanceTotal += rc1.Height();
      }
      for (int col = 0; col < cColumns; col++) {
        const int id = nIdFirstCtl + (row * cColumns) + col;
        auto wnd = parent.GetDlgItem(id);
        if (!wnd) {
          ASSERT(FALSE);
          bSuccess = false;
          continue;
    ...
    if (cyVertDistanceTotal > 0) {
      for (int col = 0; col < cColumns; col++) {
        ...
        MoveControlDelta(*wnd, 0, -cyVertDistanceTotal);
      }
    }
  }
  return bSuccess;
