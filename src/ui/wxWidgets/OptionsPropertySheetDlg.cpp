...
wxPanel* OptionsPropertySheetDlg::CreateBackupsPanel(const wxString& title)
{
  ...
  for (int i = 0; i < int(sizeof(BACKUP_SUFFIX)/sizeof(BACKUP_SUFFIX[0])); ++i) {
    Backup_SuffixCBStrings.Add(wxGetTranslation(BACKUP_SUFFIX[i]));
  }
...
wxPanel* OptionsPropertySheetDlg::CreateMiscellaneousPanel(const wxString& title)
{
  ...
  for (int i = 0; i < int(sizeof(DCAStrings)/sizeof(DCAStrings[0])); ++i) {
    wxString tmp = wxGetTranslation(DCAStrings[i]);
...
void OptionsPropertySheetDlg::PrefsToPropSheet()
{
  ...
  m_Misc_DoubleClickActionCB->SetValue(wxGetTranslation(DCAStrings[m_DoubleClickAction]));
  ...
  m_Misc_ShiftDoubleClickActionCB->SetValue(wxGetTranslation(DCAStrings[m_ShiftDoubleClickAction]));
...
{
  for (int i = 0; i < int(sizeof(DCAStrings)/sizeof(DCAStrings[0])); ++i)
    if (str == wxGetTranslation(DCAStrings[i])) {
      return i;
    }
  ASSERT(0);
  return -1;
