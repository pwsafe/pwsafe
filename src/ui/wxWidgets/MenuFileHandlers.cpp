...
void PasswordSafeFrame::OnImportText(wxCommandEvent& evt)
{
  ...
  CReport rpt;
  rpt.StartReport(L"Import Text", m_core.GetCurFile().c_str());
  wxString header;
...
void PasswordSafeFrame::OnImportXML(wxCommandEvent& evt)
{
  ...
  /* Create report as we go */
  CReport rpt;
  rpt.StartReport(L"Import XML", m_core.GetCurFile().c_str());
...
void PasswordSafeFrame::OnImportKeePass(wxCommandEvent& evt)
{
  ...
  if (ImportType == KeePassCSV)
    rpt.StartReport(L"Import KeePassV1 CSV", m_core.GetCurFile().c_str());
  else
    rpt.StartReport(L"Import KeePassV1 TXT", m_core.GetCurFile().c_str());
...
void PasswordSafeFrame::Merge(const StringX &sx_Filename2, PWScore *pothercore, const SelectionCriteria& selection)
{
  ...
  rpt.StartReport(L"Merge", m_core.GetCurFile().c_str());
