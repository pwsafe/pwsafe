 ...
 *
 * This is a cut down version of TcxMsgBox by Thales P. Carvalho
...
void CGeneralMsgBox::CreateRtfCtrl()
{
  ...
  m_strMsg.Trim();
  /*
    Phase 1 only supports plain text.
    Phase 2 will add support for HTML formatting by converting it to RTF

  CString cs_Left6 = m_strMsg.Left(6);
  CString cs_Right7 = m_strMsg.Right(7);
  if (cs_Left6.CompareNoCase(_T("<html>")) &&
      cs_Right7.CompareNoCase(_T("</html>")) {

    CString csRTF, csHTML;
    // Strip off html tags
    csHTML = m_strMsg.Mid(6, m_strMsg.GetLength() - 13);
    // Convert HTML to RTF
    HTML2RTF(m_strMsg, csRTF);

    // Load the RTF message
    EDITSTREAM es;
    es.dwCookie = (DWORD_PTR)(LPCTSTR)csRTF);
    es.dwError = 0;
    es.pfnCallback = _LoadRtfCallback;
    m_edCtrl.StreamIn(SF_RTF, es);
  } else {
    // Load a plain text message - much simpler!
