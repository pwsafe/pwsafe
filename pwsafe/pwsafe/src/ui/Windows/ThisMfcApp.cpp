...
ThisMfcApp::ThisMfcApp() :
  ...
  m_bPermitTestdump(false), m_hInstResDLL(NULL), m_CFGOpt(0)
{
      ...
      if ((*arg)[0] == L'-') {
        ...
        case L'U': case L'u': // set effective user
          ...
          if ((arg + 1) == argvec.end() || (arg + 1)[0] == L'-') {
            ...
            m_CFGOpt |= CFG_USERNAME_SUPPLIED;
          }
          break;
        case L'H': case L'h': // set effective host
          ...
          if ((arg + 1) == argvec.end() || (arg + 1)[0] == L'-') {
            ...
            m_CFGOpt |= CFG_HOSTNAME_SUPPLIED;
          }
          break;
        case L'G': case L'g': // override default config file
          ...
          if ((arg + 1) == argvec.end() || (arg + 1)[0] == L'-') {
            ...
            m_CFGOpt |= CFG_CONFIGFILE_SUPPLIED;
          }
          break;
...
BOOL ThisMfcApp::InitInstance()
{
  ...
  */
  ...
  // HOWEVER - Must deal with migration before we do this as creating PWSprefs will
  // read the config file as part of its constructor!

  // 'false' return from PerformConfigMigration means - exit PasswordSafe now
  if (!PerformConfigMigration())
    return FALSE;
  ...
  */
  ...
  m_TrayIcon = new CSystemTray((CWnd *)m_maindlg, WM_ICON_NOTIFY, L"PasswordSafe",
...
bool ThisMfcApp::PerformConfigMigration()
{
  // Do not touch U3 installations
  if (SysInfo::IsUnderU3())
    return true;
  ...
  wsHKCU = hostname.c_str();
  wsHKCU += L"\\";
  wsHKCU += username.c_str();
  wsHKCU_PREF = wsHKCU + L"\\Preferences";

  ASSERT((wsCnfgFile.empty() && m_CFGOpt == 0) || 
         (!wsCnfgFile.empty() && (m_CFGOpt & CFG_CONFIGFILE_SUPPLIED) != 0));
  ...
  if (m_CFGOpt != 0) {
    ...
    if ((m_CFGOpt & CFG_CONFIGFILE_SUPPLIED) != 0) {
    ...
    if ((m_CFGOpt & CFG_USERNAME_SUPPLIED) != 0 ||
        (m_CFGOpt & CFG_HOSTNAME_SUPPLIED) != 0) {
