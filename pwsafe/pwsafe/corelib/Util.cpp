         ...
         const unsigned char *TERMINAL_BLOCK)
{
  ...
  if (length < 0) { // sanity check
    TRACE(_T("_readcbc: Read negative length - aborting\n"));
    buffer = NULL;
    buffer_len = 0;
                     ...
                     HWND hWindow)
{
  ...
  if (OpenClipboard(hWindow) == TRUE) {
    if (EmptyClipboard() != TRUE) {
      TRACE(_T("The clipboard was not emptied correctly"));
    }
    if (SetClipboardData(CLIPBOARD_TEXT_FORMAT, hGlobalMemory) == NULL) {
      TRACE(_T("The data was not pasted into the clipboard correctly"));
    ...
    if (CloseClipboard() != TRUE) {
      TRACE(_T("The clipboard could not be closed"));
    }
  } else {
    TRACE(_T("The clipboard could not be opened correctly"));
    GlobalFree(hGlobalMemory); // wasn't passed to Clipboard
  }
  return b_retval;
}
