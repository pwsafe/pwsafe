...
struct XMLFilterWriterToFile {
  ...
  {
    string xml = PWSFilters::GetFilterXML(p.second, true);
...
string PWSFilters::GetFilterXML(const st_filters &filters, bool bFile)
{
...
void PWSFilters::GetFilterTestXML(const st_FilterData &st_fldata, ostringstream &oss,
                                  bool bFile)
{
  CUTF8Conv utf8conv;
  const unsigned char *utf8 = NULL;
